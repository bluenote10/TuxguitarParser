 ...
 * @author julian
 * 
 * FK: This class seams to be responsible for all mapping from TG datatypes to actual midi events
 * This class is (mainly) only constructed by MidiPlayer.addSequence
 */
	...
	private void makeNotes(MidiSequenceHandler sequence, TGTrack track, TGBeat beat, TGTempo tempo, int measureIdx,int bIndex, long startMove, int[] stroke) {
		...
		for( int vIndex = 0; vIndex < beat.countVoices(); vIndex ++ ){
			...
			for (int noteIdx = 0; noteIdx < voice.countNotes(); noteIdx++) {
				...
				if (!note.isTiedNote()) {
				  // FK: this was originally just this:
				  //
				  ...
				  // also the percussion track, which does not make sense.
				  // The following modification attempts the make 'transpose' more usable.
